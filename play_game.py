...
class GUI:
    ...
    def render(self, action):
        """
        根据给定的动作更新 GUI 中的 BeeBot 状态。
        :param action: BeeBot 执行的动作，可能是 FORWARD, REVERSE, SPIN_LEFT, SPIN_RIGHT 中的一种
        """
        success, cost, new_state = self.game_env.perform_action(self.state, action)
        if success:
            self.total_cost += cost
            self.state = new_state
            self.draw_hexagonal_grid()  # 重新绘制背景网格
            self.draw_environment()  # 更新环境
            self.update_status_bar()  # 更新状态栏
            self.window.update()  # 刷新窗口以显示更改
