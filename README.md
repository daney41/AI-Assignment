...
The DQN network structure is controlled by command line argument `-n ` (with choices of `s` for a single hidden layer, `d` for 2 hidden layers, and `dd` for Duelling DQN).

It also saves the trained model in [saved_models/](saved_models/).

    python dqn_gym.py -e CartPole-v1 -n d
    python dqn_bym.py -e CartPole-v1 -n dd
...
#### LunarLander-v2 Environment
    ...
    python dqn_gym.py -e LunarLander-v2 -n s
    python dqn_gym.py -e LunarLander-v2 -n d
    python dqn_gym.py -e LunarLander-v2 -n dd
